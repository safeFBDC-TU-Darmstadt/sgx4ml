...
head_row = ['Model', 'Threads', 'Batch Size', 'Min Time', 'Max Time', 'Avg Time']
...
for threads in [1, 4, 16, 32]:
    ...
    for m in models:
        ...
        for batch_size in [1, 16, 1024]:
            ...
            sum_time = 0
            train_file = open("times/" + "training_" + m + "_" + str(threads) + "_" + str(batch_size) + ".csv", "w")
            ...
            for itr, (features, targets) in enumerate(get_training_dataloader(batch_size, False)):
                ...
                if itr % 10 == 0:
                    print("Time to infer with {:2d} threads and {:04d} sized batch:{:10.7f}".format(threads, batch_size,
                                                                                                    exec_time))
            ...
            print("======================================")
            print("{:2d} threads, {:4d} batch size: min time {:10.7f}, max time {:10.7f}, avg time {:10.7f}"
                  .format(threads, batch_size, min_time, max_time, avg_time))
            writer.writerow([m, threads, batch_size, f'{min_time:.7f}', f'{max_time:.7f}', f'{avg_time:.7f}'])
            print("======================================")
